...
private:
  /// 一意(重複がない)なパケットID / Unique packet ID 
  struct UniqueId{
    /// IDは、単一アドレスコンテキスト(ex.送信者アドレス)において一意であると想定される / ID is supposed to be unique in single address context (e.g. sender address)
    Ipv4Address m_context;
    /// ID / The id
    uint32_t m_id;
    /// レコードが期限切れになるとき / When record will expire
    Time m_expire;
  };
  /// 有効期間ないであるのかを判定しているのか？
  struct IsExpired{
    bool operator() (const struct UniqueId & u) const{
      return (u.m_expire < Simulator::Now ());
    }
  };
  /// すでに見たID / Already seen IDs
  std::vector<UniqueId> m_idCache; // キューになっている？？？
  /// IDレコードのデフォルト有効期間 / Default lifetime for ID records
  Time m_lifetime;
};
