...
#include "ns3/test.h"
#include "ns3/application.h" // 追加
#include "ns3/mobility-model.h" // 追加
#include "ns3/vector.h" // 追加
...
public:
  ...
  void SetRreqLoad(uint8_t myRreqLoad){ m_myRreqLoad = myRreqLoad; }
  uint8_t GetRreqLoad(){ return m_myRreqLoad; }
  void IncreaseRreqLoad(){ m_myRreqLoad++; }
  void SetRrepLoad(uint8_t myRrepLoad){ m_myRrepLoad = myRrepLoad; }
  uint8_t GetRrepLoad(){ return m_myRrepLoad; }
  void IncreaseRrepLoad(){ m_myRrepLoad++; }
  void IncreaseMyAppLoad(Ipv4Address id); // グローバル変数の値を増加させる
  void DecreaseMyAppLoad(Ipv4Address id); // グローバル変数の値を増加させる
  void PrintMyLoad ();
  ...
  */
   ...
   * \param protocol the protocol numberfb
   */
...
private:
   ...
   */
  uint8_t m_myRreqLoad;                                 ///< RREQの負荷(追加)
  uint8_t m_myRrepLoad;                                 ///< RREPの負荷(追加)
