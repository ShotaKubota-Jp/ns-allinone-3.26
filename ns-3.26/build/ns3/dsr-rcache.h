...
struct Link {
  ...
  bool operator < (Link const& L) const {
    if (m_low < L.m_low) return true;
    else if (m_low == L.m_low) return (m_high < L.m_high);
    else return false;
  }
  void Print () const;
};

class DsrLinkStab // stability
{
public:
  // \brief Constructor
  ...
  void SetLinkStability (Time linkStab){ m_linkStability = linkStab + Simulator::Now (); }
...
private:
   ...
   * 予想されるリンク安定性寿命は、新しい情報を受け取ったときに更新する前にノードとリンクキャッシュをパージしたときに期限切れになる.
   */
...
public:
  ...
  virtual ~DsrNodeStab ();
  void SetNodeStability (Time nodeStab) { m_nodeStability = nodeStab + Simulator::Now (); }
...
public:
  ...
  void SetStabilityDecrFactor (uint64_t decrFactor) { m_stabilityDecrFactor = decrFactor; }
  uint64_t GetStabilityIncrFactor () const { return m_stabilityIncrFactor; }
  void SetStabilityIncrFactor (uint64_t incrFactor) { m_stabilityIncrFactor = incrFactor; }
  ...
  void Clear () { m_routeEntryVector.erase (m_routeEntryVector.begin (), m_routeEntryVector.end ()); }
  ...
  void SetCallback (Callback<void, Ipv4Address, uint8_t > cb) { m_handleLinkFailure = cb; }
  /// Handle link failure callback
  Callback<void, Ipv4Address, uint8_t > GetCallback () const { return m_handleLinkFailure; }
