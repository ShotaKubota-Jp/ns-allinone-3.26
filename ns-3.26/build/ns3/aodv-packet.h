...
public:
  /// c-tor
  TypeHeader (MessageType t = AODVTYPE_RREQ); // コンストラクタ

  // Header serialization/deserialization
  static TypeId GetTypeId (); // パケットヘッダのタイプIDを取得する
  TypeId GetInstanceTypeId () const; // タイプIDのインスタンスを取得する
  uint32_t GetSerializedSize () const; // パケットタイプヘッダのカプセル化サイズを取得する
  void Serialize (Buffer::Iterator start) const; // パケットタイプヘッダのカプセル化処理
  uint32_t Deserialize (Buffer::Iterator start); // パケットタイプデヘッダのカプセル化処理
...
public:
  ...
  RreqHeader (
    uint8_t flags = 0,       // タイプ
    uint8_t reserved = 0,    // 予約済み
    uint8_t hopCount = 0,    // ホップ数
    uint32_t requestID = 0,  // RREQID
    Ipv4Address dst = Ipv4Address (),    // 送信先IPアドレス
    uint32_t dstSeqNo = 0,   // 送信先シーケンス番号
    Ipv4Address origin = Ipv4Address (), // 送信元IPアドレス
    uint32_t originSeqNo = 0 // 送信元シーケンス番号
  );
...
public:
  /// c-tor
  RrepHeader (
    uint8_t prefixSize = 0,              // プレフィックス
    uint8_t hopCount = 0,                // ホップ数
    Ipv4Address dst = Ipv4Address (),    // 宛先IPアドレス
    uint32_t dstSeqNo = 0,               // 送信先シーケンス番号
    Ipv4Address origin = Ipv4Address (), // 送信IPアドレス
    Time lifetime = MilliSeconds (0),     // 生存時間
    Ipv4Address routeInfo = Ipv4Address ()
  );
  ...
  void AddRouteInfo (Ipv4Address routeInf); // 経路情報に経路を入れる
  void AddRouteInfo2 (Ipv4Address routeInf); // 経路情報に経路を入れる
  std::vector<Ipv4Address> GetRouteInfo () const { return m_routeInfo; };
  void PrintRouteInfo (); // 経路情報出力
...
private:
  ...
  std::vector<Ipv4Address> m_routeInfo; ///< 経路情報
};
