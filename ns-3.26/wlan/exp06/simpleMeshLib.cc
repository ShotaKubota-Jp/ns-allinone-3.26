...
//		LOG_PREFIX_NODE|LOG_PREFIX_TIME|LOG_PREFIX_FUNC)); 
	...
	if (m_chan) mesh.SetSpreadInterfaceChannels (MeshHelper::SPREAD_CHANNELS);
	else mesh.SetSpreadInterfaceChannels (MeshHelper::ZERO_CHANNEL);
...
MeshDot11sSim::InstallInternetStack ()
{
	...
	bgrStatic->AddNetworkRouteTo (Ipv4Address ("10.10.1.0"), Ipv4Mask ("255.255.255.0"), Ipv4Address ("192.168.1.2"), 1);
...
MeshDot11sSim::RunSim (int argc, char *argv[])
{
	...
	//SetUpUdpApplication();
    for (uint16_t i = 1; i < m_xSize*m_ySize; ++i) SetUpTcpApplication(n[i], bgr, "TCP");

	// Install FlowMonitor on all nodes
	monitor = flowmon.InstallAll();

	if(m_showRtable) Simulator::Schedule (Seconds(m_totalTime), &MeshDot11sSim::showHwmpRoutingTables, this);
