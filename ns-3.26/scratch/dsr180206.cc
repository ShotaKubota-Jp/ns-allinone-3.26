  ...
  )
{
  ...
  myURV->SetAttribute ("Min", DoubleValue (1000));
  myURV->SetAttribute ("Max", DoubleValue (2000));
  if (pktCount > 0){
    packetsSent++;
...
main(int argc, char **argv)
{
  ...
  double totalTime = 60.1; // シミュレーション時間
  ...
  for( int i=0; i<=1; i++ ){
    ...
    source->Connect (remote);
    Simulator::Schedule (Seconds (totalTime/2), &GenerateTraffic, source, packetSize, totalPackets, interPacketInterval/4);
  }
