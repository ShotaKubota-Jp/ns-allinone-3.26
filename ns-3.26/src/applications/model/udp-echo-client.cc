...
UdpEchoClient::Send (void)
{
  ...
  if (Ipv4Address::IsMatchingType (m_peerAddress))
    {
      std::cout << "UdpEchoClient::At time " << Simulator::Now ().GetSeconds () << "s client sent " << m_size << " bytes to " <<
  ...
  while ((packet = socket->RecvFrom (from)))
    {
      if (InetSocketAddress::IsMatchingType (from))
        {
          std::cout << "UdpEchoClient::At time " << Simulator::Now ().GetSeconds () << "s client received " << packet->GetSize () << " bytes from " <<
