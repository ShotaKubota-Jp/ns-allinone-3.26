...
DsrNetworkQueue::Enqueue (DsrNetworkQueueEntry & entry)
{
  ...
  m_size++;
  std::cout << "DsrNetworkQueue::Enqueue->Time:" << now << std::endl;
  //std::cout << "DsrNetworkQueue::Enqueue->(キューにエントリをプッシュ)The network queue size is " << m_size << std::endl;
  return true;
}
...
DsrNetworkQueue::Dequeue (DsrNetworkQueueEntry & entry)
{
  ...
  if (i == m_dsrNetworkQueue.end ()){
    ...
    std::cout << "DsrNetworkQueue::Dequeue->No queued packet in the network queue" << std::endl;
    return false;
  }
  entry = *i;
...
{
  NS_LOG_FUNCTION (this);
  if (m_dsrNetworkQueue.empty ()) return;
  ...
  for (std::vector<DsrNetworkQueueEntry>::iterator i = m_dsrNetworkQueue.begin (); i != m_dsrNetworkQueue.end (); ){
    if (i->GetInsertedTimeStamp () + m_maxDelay > now){
      ...
      std::cout << "DsrNetworkQueue::Cleanup->Outdated packet" << std::endl;
      i = m_dsrNetworkQueue.erase (i);
      n++;
    }
  }
