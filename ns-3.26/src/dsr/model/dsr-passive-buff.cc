...
{
  Purge ();
  for (std::vector<DsrPassiveBuffEntry>::const_iterator i = m_passiveBuffer.begin (); i != m_passiveBuffer.end (); ++i){
    /*std::cout << "DsrPassiveBuffer::Enqueue->PacketID:" << i->GetPacket ()->GetUid () << " " << entry.GetPacket ()->GetUid () << ", Source:" << i->GetSource () << " " << entry.GetSource ()
              << ", Destination:" << i->GetDestination () << " " << entry.GetDestination () << ", Identification:" << i->GetIdentification () << " "
              << entry.GetIdentification () << ", Fragment " << i->GetFragmentOffset () << " " << entry.GetFragmentOffset ()
              << ", SegLeft " << i->GetSegsLeft () << " " << entry.GetSegsLeft () << std::endl;*/
...
DsrPassiveBuffer::AllEqual (DsrPassiveBuffEntry & entry)
{
  for (std::vector<DsrPassiveBuffEntry>::iterator i = m_passiveBuffer.begin (); i != m_passiveBuffer.end (); ++i){
    /*std::cout << "DsrPassiveBuffer::AllEqual->PacketID:" << i->GetPacket ()->GetUid () << " " << entry.GetPacket ()->GetUid () << ", Source:" << i->GetSource () << " " << entry.GetSource ()
              << ", Destination:" << i->GetDestination () << " " << entry.GetDestination () << ", Identification:" << i->GetIdentification () << " "
              << entry.GetIdentification () << ", Fragment:" << i->GetFragmentOffset () << " " << entry.GetFragmentOffset ()
              << ", SegLeft:" << (uint32_t) i->GetSegsLeft () << " " << (uint32_t) entry.GetSegsLeft () << std::endl;*/
    if ((i->GetPacket ()->GetUid () == entry.GetPacket ()->GetUid ()) && (i->GetSource () == entry.GetSource ()) && (i->GetNextHop () == entry.GetNextHop ())
        && (i->GetDestination () == entry.GetDestination ()) && (i->GetIdentification () == entry.GetIdentification ()) && (i->GetFragmentOffset () == entry.GetFragmentOffset ())
        && (i->GetSegsLeft () == entry.GetSegsLeft () + 1)){
...
DsrPassiveBuffer::Dequeue (Ipv4Address dst, DsrPassiveBuffEntry & entry)
{
  ...
  for (std::vector<DsrPassiveBuffEntry>::iterator i = m_passiveBuffer.begin (); i != m_passiveBuffer.end (); ++i){
    if (i->GetDestination () == dst){
      ...
      //std::cout << "DsrPassiveBuffer::Dequeue->Packet size while dequeuing " << entry.GetPacket ()->GetSize () << std::endl;
      return true;
    }
  }
  return false;
...
DsrPassiveBuffer::Find (Ipv4Address dst)
{
  ...
  for (std::vector<DsrPassiveBuffEntry>::const_iterator i = m_passiveBuffer.begin (); i != m_passiveBuffer.end (); ++i){
    if (i->GetDestination () == dst){
      std::cout << "DsrPassiveBuffer::Find->Found the packet" << std::endl;
      return true;
    }
  }
  return false;
...
DsrPassiveBuffer::Purge ()
{
  ...
  //std::cout << "The passive buffer size " << m_passiveBuffer.size () << std::endl;
  IsExpired pred;
  for (std::vector<DsrPassiveBuffEntry>::iterator i = m_passiveBuffer.begin (); i != m_passiveBuffer.end (); ++i){
    if (pred (*i)){
      //std::cout << "DsrPassiveBuffer::Purge->Dropping Queue Packets" << std::endl;
      Drop (*i, "Drop out-dated packet ");
    }
  }
  m_passiveBuffer.erase (std::remove_if (m_passiveBuffer.begin (), m_passiveBuffer.end (), pred), m_passiveBuffer.end ());
}
...
DsrPassiveBuffer::Drop (DsrPassiveBuffEntry en, std::string reason)
{
  ...
  //en.GetErrorCallback () (en.GetPacket (), en.GetDestination (), Socket::ERROR_NOROUTETOHOST);
  return;
}
...
{
  std::cout << reason << en.GetPacket ()->GetUid () << " " << en.GetSource () << " " << en.GetNextHop () << std::endl;
  //en.GetErrorCallback () (en.GetPacket (), en.GetDestination (), Socket::ERROR_NOROUTETOHOST);
  return;
}
}  // namespace dsr
