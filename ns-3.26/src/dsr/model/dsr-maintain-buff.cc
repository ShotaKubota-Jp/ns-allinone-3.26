...
{
  Purge ();
  for (std::vector<DsrMaintainBuffEntry>::const_iterator i = m_maintainBuffer.begin (); i != m_maintainBuffer.end (); ++i){
    std::cout << "DsrMaintainBuffer::Enqueue->nexthop " << i->GetNextHop () << " " << entry.GetNextHop () << " our add " << i->GetOurAdd () << " " << entry.GetOurAdd ()
                << " src " << i->GetSrc () << " " << entry.GetSrc () << " dst " << i->GetDst () << " " << entry.GetDst ()
                << " ackId " << i->GetAckId () << " " << entry.GetAckId () << " SegsLeft " << (uint32_t)i->GetSegsLeft () << " " << (uint32_t)entry.GetSegsLeft () << std::endl;
    ...
    if ((i->GetNextHop () == entry.GetNextHop ()) && (i->GetOurAdd () == entry.GetOurAdd ()) && (i->GetSrc () == entry.GetSrc ())
      ...
      std::cout << "DsrMaintainBuffer::Enqueue->Same maintenance entry found" << std::endl;
      return false;
    }
  }
  ...
  if (m_maintainBuffer.size () >= m_maxLen){
    std::cout << "DsrMaintainBuffer::Enqueue->Drop the most aged packet" << std::endl;
...
DsrMaintainBuffer::DropPacketWithNextHop (Ipv4Address nextHop)
{
  ...
  Purge ();
  std::cout << "DsrMaintainBuffer::DropPacketWithNextHop->Drop Packet With next hop " << nextHop << std::endl;
...
DsrMaintainBuffer::Dequeue (Ipv4Address nextHop, DsrMaintainBuffEntry & entry)
{
  ...
  for (std::vector<DsrMaintainBuffEntry>::iterator i = m_maintainBuffer.begin (); i != m_maintainBuffer.end (); ++i){
    if (i->GetNextHop () == nextHop){
      ...
      std::cout << "DsrMaintainBuffer::Dequeue->Packet size while dequeuing " << entry.GetPacket ()->GetSize () << std::endl;
      return true;
    }
  }
  return false;
...
DsrMaintainBuffer::Find (Ipv4Address nextHop)
{
  for (std::vector<DsrMaintainBuffEntry>::const_iterator i = m_maintainBuffer.begin (); i != m_maintainBuffer.end (); ++i){
    if (i->GetNextHop () == nextHop){
      std::cout << "DsrMaintainBuffer::Find->DsrMaintainBuffer::Find->Found the packet in maintenance buffer" << std::endl;
      return true;
    }
  }
  return false;
...
{
  for (std::vector<DsrMaintainBuffEntry>::iterator i = m_maintainBuffer.begin (); i != m_maintainBuffer.end (); ++i){
    std::cout << "DsrMaintainBuffer::AllEqual->nexthop " << i->GetNextHop () << " " << entry.GetNextHop () << " our address " << i->GetOurAdd () << " " << entry.GetOurAdd ()
              << " src " << i->GetSrc () << " " << entry.GetSrc () << " dst " << i->GetDst () << " " << entry.GetDst ()
              << " ackId " << i->GetAckId () << " " << entry.GetAckId () << std::endl;
...
{
  for (std::vector<DsrMaintainBuffEntry>::iterator i = m_maintainBuffer.begin (); i != m_maintainBuffer.end (); ++i){
    std::cout << "DsrMaintainBuffer::NetworkEqual->nexthop " << i->GetNextHop () << " " << entry.GetNextHop () << " our address " << i->GetOurAdd () << " " << entry.GetOurAdd ()
              << " src " << i->GetSrc () << " " << entry.GetSrc () << " dst " << i->GetDst () << " " << entry.GetDst ()
              << " ackId " << i->GetAckId () << " " << entry.GetAckId () << std::endl;
    if ((i->GetOurAdd () == entry.GetOurAdd ()) && (i->GetNextHop () == entry.GetNextHop ())
        && (i->GetSrc () == entry.GetSrc ()) && (i->GetDst () == entry.GetDst ())
...
DsrMaintainBuffer::PromiscEqual (DsrMaintainBuffEntry & entry)
{
  ...
  for (std::vector<DsrMaintainBuffEntry>::iterator i = m_maintainBuffer.begin (); i != m_maintainBuffer.end (); ++i){
    std::cout << "DsrMaintainBuffer::PromiscEqual->src " << i->GetSrc () << " " << entry.GetSrc () << " dst " << i->GetDst () << " " << entry.GetDst ()
              << " SegsLeft " << (uint32_t)i->GetSegsLeft () << " " << (uint32_t)entry.GetSegsLeft () << " ackId " << (uint32_t)i->GetAckId () << " "
              << (uint32_t)entry.GetAckId () << std::endl;
...
{
  NS_LOG_DEBUG ("The maintenance buffer size " << m_maintainBuffer.size ());
  //std::cout << "DsrMaintainBuffer::LinkEqual->MaintenanceBufferSize:" << m_maintainBuffer.size () << std::endl;
  for (std::vector<DsrMaintainBuffEntry>::iterator i = m_maintainBuffer.begin (); i != m_maintainBuffer.end (); ++i){
    std::cout << "DsrMaintainBuffer::LinkEqual->Source:" << i->GetSrc () << ", Destination:" << i->GetDst () << ", OurAddress:" << i->GetOurAdd () << ", NextHop:" << i->GetNextHop () << std::endl;
    if ((i->GetSrc () == entry.GetSrc ()) && (i->GetDst () == entry.GetDst ()) && (i->GetOurAdd () == entry.GetOurAdd ()) && (i->GetNextHop () == entry.GetNextHop ())){
      i = m_maintainBuffer.erase (i);   // Erase the same maintain buffer entry for the promisc received packet
      return true;
    }
  }
...
DsrMaintainBuffer::Purge ()
{
  ...
  IsExpired pred;
  m_maintainBuffer.erase (std::remove_if (m_maintainBuffer.begin (), m_maintainBuffer.end (), pred), m_maintainBuffer.end ());
}
