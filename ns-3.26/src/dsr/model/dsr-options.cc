...
bool 
DsrOptions::ContainAddressAfter (
  ...
  )
{
  ...
  if ((vec.size () == vec2.size ()) && (vec.front () == vec2.back ())) return true;
  return false;
}

Ipv4Address 
DsrOptions::SearchNextHop (
  Ipv4Address ipv4Address, 
  std::vector<Ipv4Address>& vec
  )
{
  NS_LOG_FUNCTION (this << ipv4Address);
  Ipv4Address nextHop;
  //std::cout << "DsrOptions::SearchNextHop->The Vector Size:" << vec.size () << std::endl;
  if (vec.size () == 2){
    std::cout << "DsrOptions::SearchNextHop->The two nodes are neighbors" << std::endl;
    nextHop = vec[1];
    return nextHop;
  } else {
    if (ipv4Address == vec.back ()){
      std::cout << "DsrOptions::SearchNextHop->We have reached to the final Destination:" << ipv4Address << ", VectorSize:" << vec.back () << std::endl;
      return ipv4Address;
    }
  ...
  std::cout << "DsrOptions::SearchNextHop->ネクストホップアドレスが見つからない.経路が壊れている." << std::endl;
  Ipv4Address none = "0.0.0.0";
  return none;
}

Ipv4Address 
DsrOptions::ReverseSearchNextHop (
  Ipv4Address ipv4Address, 
  std::vector<Ipv4Address>& vec
  )
{
  ...
  Ipv4Address nextHop;
  if (vec.size () == 2){
    std::cout << "DsrOptions::ReverseSearchNextHop->The two nodes are neighbors" << std::endl;
    nextHop = vec[0];
    return nextHop;
  } else {
  ...
  std::cout << "DsrOptions::ReverseSearchNextHop->next hop address not found, route corrupted" << std::endl;
  Ipv4Address none = "0.0.0.0";
  return none;
}

Ipv4Address 
DsrOptions::ReverseSearchNextTwoHop (
  Ipv4Address ipv4Address,
  std::vector<Ipv4Address>& vec
  )
{
  ...
  //std::cout << "DsrOptions::ReverseSearchNextTwoHop->The vector size " << vec.size () << std::endl;
  ...
  std::cout << "DsrOptions::ReverseSearchNextTwoHop->next hop address not found, route corrupted" << std::endl;
  Ipv4Address none = "0.0.0.0";
  return none;
}

void 
DsrOptions::PrintVector (
  std::vector<Ipv4Address>& vec
  )
{
  NS_LOG_FUNCTION (this);
  ...
  if (!vec.size ()){
    std::cout << "DsrOptions::PrintVector->The vector is empty." << std::endl;
  } else {
    //std::cout << "DsrOptions::PrintVector->Print all the elements in a vector" << std::endl;
    std::cout << "=====VectorIPAddress:[";
    for (std::vector<Ipv4Address>::const_iterator i = vec.begin (); i != vec.end (); ++i){
      std::cout << *i << ", ";
    }std::cout << "]" << std::endl;
  }
  ...
  )
{
  ...
  //std::cout << "DsrOptionPad1::Process->Source" << source << std::endl;
...
{
  NS_LOG_FUNCTION (this << packet << dsrP << ipv4Address << source << ipv4Header << (uint32_t)protocol << isPromisc);
  //std::cout << "DsrOptionPadn::Process->Source:" << source << std::endl;
...
{
  NS_LOG_FUNCTION (this << packet << dsrP << ipv4Address << source << ipv4Header << (uint32_t)protocol << isPromisc);
  Ipv4Address srcAddress = ipv4Header.GetSource (); // Fields from IP header

  // when the ip source address is equal to the address of our own, this is request packet originated by the node itself, discard it
  if (source == ipv4Address){ // パケットが重複している
    std::cout << "DsrOptionRreq::Process->Discard the packet since it was originated from same source address." << std::endl;
  ...
  std::cout << "DsrOptionRreq::Process->Source:" << source << ", SenderIP:" << srcAddress << ", ReceiverIP:" << ipv4Address << std::endl;
  if (numberAddress >= 255) {
    std::cout << "DsrOptionRreq::Process->Discard the packet, malformed header since two many ip addresses in route" << std::endl;
    m_dropTrace (packet); // call the drop trace to show in the tracing.
    return 0;
  }

  DsrOptionRreqHeader rreq; // Create the dsr rreq header
  //  Set the number of addresses with the value from peek data and remove the rreq header.
  rreq.SetNumberAddress (numberAddress);
  p->RemoveHeader (rreq); // Remove the route request header
  uint8_t length = rreq.GetLength (); // Verify the option length
  if (length % 2 != 0){
    std::cout << "DsrOptionRreq::Process->Malformed header. Drop!" << std::endl;
    m_dropTrace (packet); // call drop trace
    return 0;
  }
  uint16_t requestId = rreq.GetId (); // (RREQID確認)Check the rreq id for verifying the request id
  // ターゲットIP-ノードのIPアドレス / The target address is where we want to send the data packets
  ...
  // Get the real source address of this request, it will be used when checking if we have received the save route request before or not
  Ipv4Address sourceAddress = nodeList.front (); // ソースノードを抽出
  //std::cout << "RREQ->";
  //PrintVector (nodeList); // 経路情報を出力
  
  // ルーティングヘッダを作成 / Construct the dsr routing header for later use
  ...
  // このリクエストを受信したかどうかをチェックし、そうでなければ、後で使用するためにリクエストをテーブルに保存します.
  // 見つからない場合はfalseを返し、新しく受信したソースリクエストエントリをキャッシュにプッシュします

  // TTLを取得 / Get the TTL value, this is used to test if the packet will be forwarded or not
  uint8_t ttl = ipv4Header.GetTtl ();
  bool dupRequest = false;  // 重複のチェック / initialize the duplicate request check value
  // if the ttl value is not 0, then this request will be forwarded, then we need to save it in the source entry
  // TTLの値が0でなければ、このリクエストは転送され、それをソースエントリに保存する必要があります.
  if (ttl) dupRequest = dsr->FindSourceEntry (sourceAddress, targetAddress, requestId);
  ...
  if (dupRequest){
    ...
    //std::cout << "DsrOptionRreq::Process->Duplicate request. Drop!" << std::endl;
    m_dropTrace (packet); // call drop trace
    return 0;
  } else if (CheckDuplicates (ipv4Address, nodeList)){
    // if the route contains the node address already, drop the request packet
    m_dropTrace (packet);    // call drop trace
    //std::cout << "DsrOptionRreq::Process->Our node address is already seen in the route, drop the request." << std::endl;
    return 0;
  } else {
    // A node ignores all RREQs received from any node in its blacklist.
    DsrRouteCacheEntry toPrev;
    bool isRouteInCache = dsr->LookupRoute (targetAddress, toPrev);
    isRouteInCache = false; // adding by shotakubota.

    DsrRouteCacheEntry::IP_VECTOR ip = toPrev.GetVector (); // 私たち自身のルートキャッシュから目的地までのルート / The route from our own route cache to dst
    std::cout << "ルートキャッシュから目的地までの経路:";
    PrintVector (ip);
    ...
    // 逆ルートが作成または更新されると、ルート上で次のアクションも実行されます.
    //  3.ルーティングテーブルの次のホップが、RREQを受信したノードになります.
    //  4.ホップカウントは、RREQメッセージのホップカウントからコピーされます.

    //  A node generates a RREP if either: (i) it is itself the destination, The target address equal to our own ip address
    // ノードは、（i）自身が宛先である場合、ターゲットアドレスが自分のIPアドレスに等しい場合、RREPを生成します。
    std::cout << "DsrOptionRreq::Process->ReceiverIP::" << targetAddress << ", SourceIP:" << ipv4Address << ", SenderIP:" << mainVector[0] << std::endl;
    if (targetAddress == ipv4Address) {
      std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
      std::cout << "~~~~~ DsrOptionRreq::Process->RREQが宛先に到達!!!!!!" << std::endl;
      std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
      Ipv4Address nextHop; // Declare the next hop address to use
      if (nodeList.size () == 1){
        std::cout << "DsrOptionRreq::Process->これらの2つのノードは隣接ノード / These two nodes are neighbors." << std::endl;
        m_finalRoute.clear ();
      ...
      std::cout << "##### DsrOptionRreq::Process->RREPヘッダを作成!!!!!!" << std::endl;
      DsrOptionRrepHeader rrep;
      rrep.SetNodesAddress (m_finalRoute);     // Set the node addresses in the route reply header
      std::cout << "DsrOptionRreq::Process->The nextHop address:[" << nextHop << "]" << std::endl;
      ...
      SetRoute (nextHop, ipv4Address); // Set the route for route reply
      ...
      // rreq発信元へのルートエントリを作成し、それをルートキャッシュに保存します。また、ルートを逆にする必要がある.
      PrintVector (m_finalRoute);
      if (ReverseRoutes (m_finalRoute)) { // 逆ルートの生成をするかどうか
        std::cout << "RREPパケット用に逆ルート生成->";
        PrintVector (m_finalRoute);
        Ipv4Address dst = m_finalRoute.back ();
        bool addRoute = false;
        if (numberAddress > 0){
          DsrRouteCacheEntry toSource (
            m_finalRoute, /*IP_VECTOR=*/
            dst, /*dst=*/
            ActiveRouteTimeout /*expire time=*/
          );
          if (dsr->IsLinkCache ()) addRoute = dsr->AddRoute_Link (m_finalRoute, ipv4Address);
          else  addRoute = dsr->AddRoute (toSource);
        } else {
          std::cout << "DsrOptionRreq::Process->Abnormal RouteRequest." << std::endl;
          return 0;
        }

        if (addRoute){
          // Found a route to the dst, construct the source route option header
          // 宛先へのルートを見つけ、ソースルートオプションヘッダーを作成
          DsrOptionSRHeader sourceRoute;
          //std::cout << "DsrOptionRreq::Process->The route length:" << m_finalRoute.size () << std::endl;
          sourceRoute.SetNodesAddress (m_finalRoute);

          /// TODO !!!!!!!!!!!!!!
          /// Think about this part, we just added the route, probability no need to increase stability now?????
          // if (dsr->IsLinkCache ())dsr->UseExtends (m_finalRoute);
          sourceRoute.SetSegmentsLeft ((m_finalRoute.size () - 2));
          sourceRoute.SetSalvage (0); // The salvage value here is 0
          Ipv4Address nextHop = SearchNextHop (ipv4Address, m_finalRoute); // Get the next hop address
          std::cout << "DsrOptionRreq::Process->NextHopAddress:" << nextHop << std::endl;
          ...
          std::cout << "DsrOptionRreq::Process->経路はキャッシュに追加できない / The route is failed to add in cache." << std::endl;
          return 0;
        }
      } else {
        std::cout << "DsrOptionRreq::Process->経路を逆転できない / Unable to reverse route." << std::endl;
        return 0;
      }
      isPromisc = false;
     ...
     *（ii）または宛先へのアクティブな経路を有し、要求ヘッダおよび経路キャッシュに基づいて応答を送信し、d = H *（h-1 + r）からのランダム値に基づいて遅延する必要があり、 返信の嵐 "です。
     * また、2つのベクトルに重複が含まれていないかどうかを確認します(経路キャッシュからの宛先への経路の一部とこれまでに収集された経路)
     * その場合、見つかったルートを使用せず、ルート要求を転送してください
     **/
      ...
      // Create the route entry to the rreq originator and save it to route cache, also need to reverse the route
      // RREQ発信者へのルートエントリを作成し、ルートキャッシュに保存します。また、ルートを逆にする必要があります
      bool addRoute = false;
      ...
      if (ReverseRoutes (reverseRoute)){
        ...
        std::cout << "DsrOptionRreq::Process->This is the route save in route cache." << std::endl;
        PrintVector (saveRoute);
        ...
        // ルートキャッシュにルートエントリを追加 / Add the route entry in the route cache
        if (dsr->IsLinkCache ()) addRoute = dsr->AddRoute_Link (saveRoute, ipv4Address);
        else addRoute = dsr->AddRoute (toSource);

        if (addRoute){
          std::cout << "DsrOptionRreq::Process->宛先を持つパケットのルートと検索送信バッファを追加 / We have added the route and search send buffer for packet with destination." << dst << std::endl;
          
          // 目的地ルートを見つけ、ソースルートオプションヘッダを構築します.
          // Found a route the dst, construct the source route option header
          DsrOptionSRHeader sourceRoute;
          PrintVector (saveRoute);

          sourceRoute.SetNodesAddress (saveRoute);
          // if (dsr->IsLinkCache ()) dsr->UseExtends (saveRoute);
          ...
          SetRoute (nextHop, ipv4Address);
          dsr->SendPacketFromBuffer (sourceRoute, nextHop, protocol); // Schedule the packet retry
          dsr->CancelRreqTimer (dst, true); // Cancel the route request timer for destination
        } else {
          std::cout << "DsrOptionRreq::Process->経路はキャッシュに追加できない / The route is failed to add in cache." << std::endl;
          return 0;
        }
      } else {
        std::cout << "DsrOptionRreq::Process->経路を逆転できない / Unable to reverse the route." << std::endl;
        return 0;
      }

      // Need to first pin down the next hop address before removing duplicates
      // 重複を削除する前に、まずネクストホップアドレスをピンダウンする必要がある.
      Ipv4Address nextHop = ReverseSearchNextHop (ipv4Address, m_finalRoute);
      // First remove the duplicate ip address to automatically shorten the route, and then reversely search the next hop address
      // 最初に重複したIPアドレスを削除してルートを自動的に短くし、次のホップアドレスを逆方向に検索
      SetRoute (nextHop, ipv4Address); // Set the route
      ...
      PrintVector (m_finalRoute);
      std::cout << "DsrOptionRreq::Process->This is the full route from " << realSource << " to " << m_finalRoute.front () << std::endl;
    ...
    // どんなタイプの経路も見つからない.
      ...
      std::cout << "DsrOptionRreq::Process->Print out the main vector:";
      PrintVector (mainVector);
      rreq.SetNodesAddress (mainVector);

      Ptr<Packet> errP = p->Copy ();
      if (errP->GetSize ()){
        std::cout << "DsrOptionRreq::Process->Error header included." << std::endl;
        ...
        if ((errorSrc == srcAddress) && (unreachNode == ipv4Address)){
          std::cout << "DsrOptionRreq::Process->The error link back to work again." << std::endl;
          uint16_t length = rreq.GetLength ();
          std::cout << "DsrOptionRreq::Process->エラー用+RREQ header length:" << length << std::endl;
          ...
          std::cout << "DsrOptionRreq::Process->サルベージ用+RREQ and newUnreach header length:" <<  length << std::endl;
        ...
        uint16_t length = rreq.GetLength ();
        std::cout << "DsrOptionRreq::Process->パケットサイズ正常+RREQ header length:" <<  length << std::endl;
        dsrRoutingHeader.AddDsrOption (rreq);
        dsrRoutingHeader.SetPayloadLength (length + 2);
      }
      uint8_t ttl = ipv4Header.GetTtl (); // Get the TTL value

      // Decrease the TTL value in the packet tag by one, this tag will go to ip layer 3 send function and drop packet when TTL value equals to 0
      // パケットタグのTTL値を1減らす. このタグは、TTL値が0に等しいときに、IPレイヤ3送信機能および廃棄パケットに移動.
      std::cout << "DsrOptionRreq::Process->TTLValue:" << (uint32_t)ttl << std::endl;
      if (ttl) {
  ...
  )
{
  ...
  std::cout << "DsrOptionRrep::Process->Source:" << source << ", NextHeaderValue:" << (uint32_t)protocol << std::endl;
  std::vector<Ipv4Address> nodeList = rrep.GetNodesAddress (); // 経路情報全てを取得

  // Get the destination address, which is the last element in the nodeList
  Ipv4Address targetAddress = nodeList.front (); // 宛先アドレスを取得(多分制御パケット)
  // If the RREP option has reached to the destination
  if (targetAddress == ipv4Address){
    std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
    std::cout << "~~~~~ DsrOptionRrep::Process->RREPパケットが宛先に到達!!!" << std::endl;
    std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
    RemoveDuplicates (nodeList); // This is for the route reply from intermediate node since we didn't remove duplicate there
    if (nodeList.size () == 0){
      std::cout << "DsrOptionRrep::Process->The route we have contains 0 entries." << std::endl;
      return 0;
    }

    //***************************************************************************
    //dsr->IncreaseRrepLoad(); // RREPの負荷を増分
    //dsr->PrintMyLoad();
    dsr->IncreaseMyAppLoad(targetAddress); // グローバル変数のRREP負荷を増分
    Ptr<ns3::Application> app;
    app->ReceiveData(nodeList); // アプリケーション層に経路情報を送信する(という想定)
    /*****************************************************************************
     * ここで引数の、nodeListで変化した場合、新しい処理を加えねばならない。
     * 現段階では、上記の処理が実装されていない
     *****************************************************************************/
    
    // Get the destination address for the data packet, which is the last element in the nodeList
    // nodeListの最後の要素であるデータパケットの宛先アドレスを取得(データパケット)
    Ipv4Address dst = nodeList.back ();
    /**
     * Add the newly found route to the route cache
     * The route looks like: "srcAddress" + "intermediate node address" + "targetAddress"
     */
    DsrRouteCacheEntry toDestination (
      /*IP_VECTOR=*/ nodeList, 
      /*dst=*/ dst, 
      /*expire time=*/ ActiveRouteTimeout
    );
    ...
    bool addRoute = false;
    if (dsr->IsLinkCache ()) addRoute = dsr->AddRoute_Link (nodeList, ipv4Address);
    else addRoute = dsr->AddRoute (toDestination);

    if (addRoute){
      std::cout << "DsrOptionRrep::Process->目的地のパケットの経路と検索送信バッファを追加." /*We have added the route and search send buffer for packet with destination */ << dst << std::endl;
      // 目的地ルートを見つけ、ソースルートオプションヘッダを構築 / Found a route the dst, construct the source route option header
      DsrOptionSRHeader sourceRoute;
      ...
      std::cout << "DsrOptionRrep::Process->RouteLength:" << nodeList.size () << ", NextHopAddress:" << nextHop << std::endl;
      if (nextHop == "0.0.0.0"){
        dsr->PacketNewRoute (dsrP, ipv4Address, dst, protocol); // 経路ベクトルが破損した場合、通常は発生していない新しいパケットを発信
        return 0;
      }
      std::cout << "経路情報->"; 
      ...
      // 多分、データパケットを送信する関数に移る
      dsr->SendPacketFromBuffer (sourceRoute, nextHop, protocol); // Schedule the packet retry
    } else {
      std::cout << "DsrOptionRrep::Process->Failed to add the route." << std::endl;
      return 0;
    }
  } else {
    std::cout << "~~~~~ DsrOptionRrep::Process->RREPが中継ノードに到達." << std::endl;
    uint8_t length = rrep.GetLength () - 2; // The get length - 2 is to get aligned for the malformed header check
    std::cout << "DsrOptionRrep::Process->The length of RREPOption:" << (uint32_t)length << std::endl;

    if (length % 2 != 0){
      std::cout << "DsrOptionRrep::Process->Malformed header. Drop!" << std::endl;
      m_dropTrace (packet);
      return 0;
    }
    std::cout << "NodeList->";
    ...
    //std::cout << "DsrOptionRrep::Process->このノードは中間ノードに過ぎないが、経路を切断するときに目的地に可能な経路を保存する必要がある" << std::endl;
    ...
    if (cutRoute.size () >= 2){
      ...
      std::cout << "DsrOptionRrep::Process->The route destination after cut " << dst << std::endl;
      DsrRouteCacheEntry toDestination (
        /*IP_VECTOR=*/ cutRoute, 
        /*dst=*/ dst, 
        /*expire time=*/ ActiveRouteTimeout
      ...
      if (addRoute) dsr->CancelRreqTimer (dst, true);
      else std::cout << "DsrOptionRrep::Process->追加されなかった経路/The route not added." << std::endl;

    } else {
      std::cout << "DsrOptionRrep::Process->経路故障." << std::endl;
    }
    ...
    std::cout << "DsrOptionRrep::Process->NextHopAddress:" << nextHop << ", and the source in the route reply " << source << std::endl;
    SetRoute (nextHop, ipv4Address); // Set the route entry we will use to send reply
    ...
    std::cout << "DsrOptionRrep::Process->ReplyHeaderLength:" << (uint32_t)length << std::endl;
    dsrRoutingHeader.SetPayloadLength (length + 2);
    dsrRoutingHeader.SetMessageType (1);
    dsrRoutingHeader.SetSourceId (GetIDfromIP (source));
  ...
  )
{
   ...
   */
  if (isPromisc){
    std::cout << "DsrOptionSR::Process->無差別受信データパケットを処理 / We process promiscuous receipt data packet." << std::endl;
    if (ContainAddressAfter (ipv4Address, destAddress, nodeList)){
      std::cout << "DsrOptionSR::Process->無償返信を送り返す / Send back the gratuitous reply." << std::endl;
    ...
    if (destAddress != destination){
      std::cout << "DsrOptionSR::Process->無差別に受信したパケットを処理 / Process the promiscuously received packet." << std::endl;
      bool findPassive = false;
      int32_t nNodes = NodeList::GetNNodes ();
      for (int32_t i = 0; i < nNodes; ++i){
        //std::cout << "DsrOptionSR::Process->Working with node " << i << std::endl;
        Ptr<Node> node = NodeList::GetNode (i);
        ...
        if (findPassive) break;
      }

      if (findPassive){
        std::cout << "DsrOptionSR::Process->我々は以前に受動的に受け入れられたものを見つける / We find one previously received passive entry." << std::endl;
        // Get the node from IP address and get the DSR extension object
        // the srcAddress would be the source address from ip header
        PrintVector (nodeList);

        std::cout << "DsrOptionSR::Process->PromiscSource:" << promiscSource << std::endl;
        ...
        std::cout << "DsrOptionSR::Process->今後、使用のためにエントリーを保存 / Saved the entry for further use." << std::endl;
    ...
    if (optionType == 160){
      std::cout << "DsrOptionSR::Process->ack要求ヘッダーを削除し、ACKヘッダーをパケットに追加 / Remove the ack request header and add ack header to the packet" << std::endl;
      ...
      if (!nodeList.empty ()){
        if (segsLeft > numberAddress){ // The segmentsLeft field should not be larger than the total number of ip addresses
          std::cout << "DsrOptionSR::Process->Malformed header. Drop!" << std::endl;
          m_dropTrace (packet);
          return 0;
        }
      ...
      std::cout << "DsrOptionSR::Process->Send back ACK to the earlier hop " << ackAddress << " from us " << ipv4Address << std::endl;
    ...
    /*
      ...
      */
    if (segsLeft == 0){
      std::cout << "##### ソースルートパケットが最終目的地に到達!!!!!!" << std::endl;
      std::cout << "DsrOptionSR::Process->最終目的地に到達 / This is the final destination." << std::endl;
      isPromisc = false;
    ...
    std::cout << "DsrOptionSR::Process->The next address of source route option " << nextAddress << " and the nextAddressIndex:" << (uint32_t)nextAddressIndex << " and the segments left:" << (uint32_t)segsLeft << std::endl;
    ...
    if (nextHop == "0.0.0.0"){
      std::cout << "DsrOptionSR::Process->Before new packet " << *dsrP << std::endl;
    ...
    if (ipv4Address == nextHop){
      std::cout << "##### ソースルートパケットが最終目的地に到達!!!!!!" << std::endl;
      std::cout << "DsrOptionSR::Process->目的地に到達 / We have reached the destination." << std::endl;
    ...
    std::cout << "DsrOptionSR::Process->DSRPacketSize:" << dsrP->GetSize () << std::endl;
...
{
  NS_LOG_FUNCTION (this << packet << dsrP << ipv4Address << source << ipv4Header << (uint32_t)protocol << isPromisc);
  std::cout << "DsrOptionRerr::Process->Source:" << source << std::endl;
  Ptr<Packet> p = packet->Copy ();
  uint32_t size = p->GetSize ();
  uint8_t *data = new uint8_t[size];
  p->CopyData (data, size);
  ...
  uint32_t rerrSize;
  std::cout << "DsrOptionRerr::Process->The error type value here " << (uint32_t)errorType << std::endl;
  if (errorType == 1){ // unreachable ip addrese
    ...
    std::cout << "DsrOptionRerr::Process->The error source is " <<  rerrUnreach.GetErrorDst () << "and the unreachable node is " << unreachAddress << std::endl;
  ...
  )
{
  ...
  std::cout << "DsrOptionRerr::DoSendError->The number of addresses " << (uint32_t)numberAddress << std::endl;
  DsrOptionSRHeader sourceRoute;
  sourceRoute.SetNumberAddress (numberAddress);
  p->RemoveHeader (sourceRoute);
  ...
  if (length % 2 != 0){
    std::cout << "DsrOptionRerr::DoSendError->Malformed header. Drop!" << std::endl;
    m_dropTrace (p);
    return 0;
  }
  ...
  if (segmentsLeft == 0 && targetAddress == ipv4Address){
    std::cout << "DsrOptionRerr::DoSendError->This is the destination of the error, send error request" << std::endl;
  ...
  )
{
  ...
  Ptr<Node> node = GetNodeWithAddress (ipv4Address);
  Ptr<dsr::DsrRouting> dsr = node->GetObject<dsr::DsrRouting> ();
  std::cout << "DsrOptionAckReq::Process->Source:" << source << ", NextHeaderValue:" << (uint32_t)protocol << std::endl;
...
{
  NS_LOG_FUNCTION (this << packet << dsrP << ipv4Address << source << ipv4Header << (uint32_t)protocol << isPromisc);
  std::cout << "DsrOptionAck::Process->Source" << source << std::endl;
