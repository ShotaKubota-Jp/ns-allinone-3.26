...
{
  Purge ();
  for (std::vector<DsrErrorBuffEntry>::const_iterator i = m_errorBuffer.begin (); i != m_errorBuffer.end (); ++i){
    /*std::cout << "packet id " << i->GetPacket ()->GetUid () << " " << entry.GetPacket ()->GetUid () << " source " << i->GetSource () << " " << entry.GetSource ()
              << " next hop " << i->GetNextHop () << " " << entry.GetNextHop () << " dst " << i->GetDestination () << " " << entry.GetDestination () << std::endl;*/
...
DsrErrorBuffer::DropPacketForErrLink (Ipv4Address source, Ipv4Address nextHop)
{
  ...
  m_errorBuffer.erase (std::remove_if (m_errorBuffer.begin (), m_errorBuffer.end (), std::bind2nd (std::ptr_fun (DsrErrorBuffer::LinkEqual), link)), m_errorBuffer.end ());
}
...
DsrErrorBuffer::Dequeue (Ipv4Address dst, DsrErrorBuffEntry & entry)
{
  ...
  for (std::vector<DsrErrorBuffEntry>::iterator i = m_errorBuffer.begin (); i != m_errorBuffer.end (); ++i){
    if (i->GetDestination () == dst){
      ...
      //std::cout << "Packet size while dequeuing " << entry.GetPacket ()->GetSize () << std::endl;
      return true;
    }
  }
  return false;
...
DsrErrorBuffer::Find (Ipv4Address dst)
{
  ...
  for (std::vector<DsrErrorBuffEntry>::const_iterator i = m_errorBuffer.begin (); i != m_errorBuffer.end (); ++i){
    if (i->GetDestination () == dst){
      //std::cout << "Found the packet" << std::endl;
      return true;
    }
  }
  return false;
...
DsrErrorBuffer::Purge ()
{
  ...
  //std::cout << "The error buffer size " << m_errorBuffer.size () << std::endl;
  IsExpired pred;
  for (std::vector<DsrErrorBuffEntry>::iterator i = m_errorBuffer.begin (); i != m_errorBuffer.end (); ++i){
    if (pred (*i)){
      std::cout << "Dropping Queue Packets" << std::endl;
      Drop (*i, "Drop out-dated packet ");
    }
  }
  m_errorBuffer.erase (std::remove_if (m_errorBuffer.begin (), m_errorBuffer.end (), pred), m_errorBuffer.end ());
}
