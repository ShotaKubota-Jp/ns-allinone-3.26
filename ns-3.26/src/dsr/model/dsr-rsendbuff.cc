...
{
  Purge ();
  for (std::vector<DsrSendBuffEntry>::const_iterator i = m_sendBuffer.begin (); i != m_sendBuffer.end (); ++i){
    std::cout << "DsrSendBuffer::Enqueue->Packet ID:" << i->GetPacket ()->GetUid () << " " << entry.GetPacket ()->GetUid () << ", Destination:" << i->GetDestination () << " " << entry.GetDestination () << std::endl;
    if ((i->GetPacket ()->GetUid () == entry.GetPacket ()->GetUid ()) && (i->GetDestination () == entry.GetDestination ())) {
      return false;
    }
  }
...
DsrSendBuffer::Dequeue (Ipv4Address dst, DsrSendBuffEntry & entry)
{
  ...
  for (std::vector<DsrSendBuffEntry>::iterator i = m_sendBuffer.begin (); i != m_sendBuffer.end (); ++i){
    if (i->GetDestination () == dst){
      ...
      std::cout << "DsrSendBuffer::Dequeue->Packet size while dequeuing " << entry.GetPacket ()->GetSize () << std::endl;
      return true;
    }
  }
  return false;
...
DsrSendBuffer::Find (Ipv4Address dst)
{
  ...
  for (std::vector<DsrSendBuffEntry>::const_iterator i = m_sendBuffer.begin (); i != m_sendBuffer.end (); ++i){
    if (i->GetDestination () == dst){
      std::cout << "DsrSendBuffer::Find->FOUNDTHEPACKET" << std::endl;
      return true;
    }
  }
  return false;
...
DsrSendBuffer::Purge ()
{
  ...
  for (std::vector<DsrSendBuffEntry>::iterator i = m_sendBuffer.begin (); i != m_sendBuffer.end (); ++i){
    if (pred (*i)){
      std::cout << "DsrSendBuffer::Purge->Dropping Queue Packets" << std::endl;
      Drop (*i, "Drop out-dated packet ");
    }
  }
  m_sendBuffer.erase (std::remove_if (m_sendBuffer.begin (), m_sendBuffer.end (), pred), m_sendBuffer.end ());
}
