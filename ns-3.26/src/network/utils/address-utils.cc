...
// 追加
/*
void WriteTo (Buffer::Iterator &i, std::vector<Ipv4Address> ad)
{
  NS_LOG_FUNCTION (&i << &ad);
  std::cout << "WriteTo" << std::endl;
  for(std::vector<Ipv4Address>::const_iterator j = ad.begin(); j != ad.end(); j++){
    //i.WriteHtonU32 (ad.Get ());
    std::cout << j->Get() << std::endl;
    //i.WriteHtonU32 (j->Get ());
  }
}
void ReadFrom (Buffer::Iterator &i, std::vector<Ipv4Address> &ad)
{
  NS_LOG_FUNCTION (&i << &ad);
  std::cout << "ReadFrom" << std::endl;
  for(std::vector<Ipv4Address>::const_iterator x = ad.begin(); x != ad.end(); x++){
