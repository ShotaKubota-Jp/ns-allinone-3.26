...
RrepHeader::RrepHeader (
  ...
  m_dstSeqNo (dstSeqNo)
{
  //m_routeInfo = new Ipv4Address[10]; // 配列の領域確保
  //for(int i=0;i<m_routeInfo.size();i++) m_routeInfo[i] = routeInfo;
  //m_routeInfo.push_back(routeInfo);
  if(routeInfo != Ipv4Address () ) m_routeInfo.push_back(routeInfo);
...
{
  std::cout << "RrepHeader::GetTypeId" << std::endl;
...
{
  return 19 + m_routeInfo.size () * 4;
}

void
RrepHeader::AddRouteInfo (Ipv4Address routeInfo)
{
  if(routeInfo != Ipv4Address() ) m_routeInfo.push_back(routeInfo);
  std::cout << "経路情報追加->Vectorサイズ:" << m_routeInfo.size() << ", 経路情報:" << routeInfo << std::endl;
}

void 
RrepHeader::SetNodeAddress (uint8_t index, Ipv4Address addr)
{
  m_routeInfo.at (index) = addr;
}

void
RrepHeader::PrintRouteInfo () {
  std::cout << "経路情報->";
  for(unsigned int i=0;i<m_routeInfo.size();i++){
    std::cout << m_routeInfo[i] << ",";
  }
  std::cout << std::endl;
}
...
{
  std::cout << "RrepHeader::Serialize" << std::endl;
  i.WriteU8 (m_flags);
  ...
  uint8_t buff[4];
  for (std::vector<Ipv4Address>::const_iterator it = m_routeInfo.begin (); it != m_routeInfo.end (); it++){
    it->Serialize (buff);
    i.Write (buff, 4);
  }
  std::cout << "HeaderSize:" << GetSerializedSize () << std::endl;
}

uint32_t
RrepHeader::Deserialize (Buffer::Iterator start)
{
  std::cout << "RrepHeader::Deserialize" << std::endl;
  Buffer::Iterator i = start;
  m_flags = i.ReadU8 ();
  ...
  uint8_t buff[4];
  uint8_t index = 0;
  for (std::vector<Ipv4Address>::iterator it = m_routeInfo.begin (); it != m_routeInfo.end (); it++){
    i.Read (buff, 4);
...
RrepHeader::Print (std::ostream &os) const
{
  ...
  if (m_prefixSize != 0) os << " prefix size " << m_prefixSize;
